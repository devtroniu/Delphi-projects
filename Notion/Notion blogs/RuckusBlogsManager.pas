...
type
  ...
  TRuckusBlogsManager = class(TNotionManager)
  protected
...
function TRuckusBlogsManager.BodyJSON(dest_id, pageName, pagePostDate, pageBody: String): string;
var
...
function TRuckusBlogsManager.CleanBody(theBody: String): string;
begin
  // remove tags
  ...
  body := StringReplace(body, '&lsquo;', '''', [rfReplaceAll]);
  body := StringReplace(body, '&nbsp;', '"', [rfReplaceAll]);
...
procedure TRuckusBlogsManager.SavePageInDatabase(dest_id: string; page: INotionPage);
var
  body: string;
begin
...
procedure TRuckusBlogsManager.MarkAsFailed(pageId: string);
var
